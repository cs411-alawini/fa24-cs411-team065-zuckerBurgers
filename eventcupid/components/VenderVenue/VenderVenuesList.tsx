...
function VVlist() {
  ...
  // Fetch hot venues
  const { data: hotVenues = [], isLoading: isLoadingHotVenues, isError: isErrorHotVenues, error: hotVenuesError } = useQuery<Venue[]>({
    queryKey: ["hotVenues"],
  ...
  // Fetch all venues
  const { data: venues = [], isLoading: isLoadingVenues, isError: isErrorVenues, error: venuesError } = useQuery<Venue[]>({
    queryKey: ["venues"],
    queryFn: fetchVenues,
  ...
  if (isLoading) {
    return <p>Loading {activeTab === 0 ? "hot venues" : "all venues"}...</p>;
  }

  if (isError) {
    return <p>Failed to load {activeTab === 0 ? "hot venues" : "all venues"}: {error?.message}. Please try again later.</p>;
  }

  if (!data || data.length === 0) {
    return <p>No {activeTab === 0 ? "hot venues" : "all venues"} available.</p>;
  }
    ...
    <div className="container my-5">
      ...
      <div className="flex justify-center mb-4">
        <div className="relative w-1/2 flex items-center">
          ...
          <button
            className={`flex-1 py-2 ${activeTab === 1 ? "bg-blue-500 text-white" : "bg-gray-300 text-black"} rounded-r-md`}
